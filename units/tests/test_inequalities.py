...
def test_invalid_gte(quant1, quant2):
    """Binary function to assert the operator's exception"""
    py.test.raises(IncompatibleUnitsError, greater_than_or_eq, quant1, quant2)
...
def pytest_generate_tests(metafunc):
    ...
    if metafunc.function == test_eq:
        metafunc.addcall(funcargs=dict(quant1=COMPATIBLE_QUANTITIES[0][0], quant2=COMPATIBLE_QUANTITIES[1][0]))
        metafunc.addcall(funcargs=dict(quant1=COMPATIBLE_QUANTITIES[1][0], quant2=COMPATIBLE_QUANTITIES[0][0]))
    ...
    if metafunc.function in [test_lte, test_lt, test_gte, test_gt]:
        ...
        metafunc.addcall(funcargs=dict(quant1=COMPATIBLE_QUANTITIES[1][0], quant2=COMPATIBLE_QUANTITIES[0][1]))
        metafunc.addcall(funcargs=dict(quant1=COMPATIBLE_QUANTITIES[0][0], quant2=COMPATIBLE_QUANTITIES[1][1]))
    
    if metafunc.function in [test_invalid_gte, test_invalid_gt, test_invalid_lte, test_invalid_lt]:
        for i, q_group1 in enumerate(QUANTITIES):
            for j in range(i + 1, len(QUANTITIES)):
                q_group2 = QUANTITIES[j]
                metafunc.addcall(funcargs=dict(quant1=q_group1[0],quant2=q_group2[0]))
