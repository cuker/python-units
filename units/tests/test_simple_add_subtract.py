...
def test_good_simple_add():
    ...
    assert (Quantity(2, unit('m')) + Quantity(2, unit('m')) == Quantity(4, unit('m')))
...
def test_good_simple_sub():
    ...
    assert (Quantity(4, unit('m')) - Quantity(2, unit('m')) == Quantity(2, unit('m')))
...
def test_good_composed_add():
    ...
    assert (Quantity(2, unit('m') / unit('s')) + Quantity(3, unit('m') / unit('s')) == Quantity(5, unit('m') / unit('s')))

def test_good_named_add():
    """Two quantities with the same named complex units should add together"""
    
    furlong = NamedComposedUnit('furlong', ComposedUnit([unit('m')], [], multiplier=201.168), is_si=False)
    
    assert (Quantity(2, furlong) + Quantity(2, furlong) == Quantity(4, furlong))
...
def test_good_mixed_add():
    ...
    gray = NamedComposedUnit('gray', ComposedUnit([unit('J'), unit('kg')], []), is_si=True)
...
def test_good_add_w_mult():
    ...
    assert(Quantity(1, moon) + Quantity(1, lunar_cycle) == Quantity(2, moon))
    
    assert(Quantity(1, lunar_cycle) + Quantity(1, moon) == Quantity(2, lunar_cycle))
...
def test_good_named_add_w_mults():
    ...
    assert within_epsilon(Quantity(1, mile) + Quantity(1, kilometre), Quantity(1, kilometre) + Quantity(1, mile))
    assert within_epsilon(Quantity(2609.344, unit('m')), Quantity(1, kilometre) + Quantity(1, mile))
...
def test_good_composed_sub():
    ...
    assert (Quantity(5, unit('m') / unit('s')) - Quantity(3, unit('m') / unit('s')) == Quantity(2, unit('m') / unit('s')))
...
def test_good_named_sub():
    ...
    assert (Quantity(4, furlong) - Quantity(2, furlong) == Quantity(2, furlong))
...
def test_good_sub_w_mult():
    ...
    assert(Quantity(2, lunar_cycle) - Quantity(1, moon) == Quantity(1, lunar_cycle))
...
def test_good_named_sub_w_mults():
    ...
    assert within_epsilon(Quantity(1, mile) - Quantity(1, kilometre), Quantity(609.344, unit('m')))
    
    assert within_epsilon(Quantity(1, kilometre) - Quantity(1, mile), Quantity(-609.344, unit('m')))
